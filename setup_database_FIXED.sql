-- =====================================================
-- KHAO PEEO RESTAURANT SYSTEM - COMPLETE DATABASE SETUP (FIXED)
-- =====================================================
-- Run this entire script in Supabase SQL Editor
-- This version fixes the enum conversion issue

-- =====================================================
-- STEP 1: Create Order Status Enum FIRST
-- =====================================================
...
-- =====================================================
-- STEP 2: Create User Role Enum (with waiter)
-- =====================================================
...
-- =====================================================
-- STEP 5: Create Orders Table (with enum type directly)
-- =====================================================

-- Drop table if it exists with wrong schema (optional - comment out if you have data)
...
-- If orders table already exists with TEXT status, convert it
DO $$
BEGIN
  -- Check if status column is TEXT (not enum)
  IF EXISTS (
    ...
    ALTER TABLE public.orders 
      ALTER COLUMN status TYPE public.order_status USING 
        CASE 
          ...
          WHEN status::text = 'sent_to_kitchen' THEN 'sent_to_kitchen'::public.order_status
          WHEN status::text = 'preparing' THEN 'preparing'::public.order_status
          WHEN status::text = 'ready' THEN 'ready'::public.order_status
          WHEN status::text = 'served' THEN 'served'::public.order_status
...
UNION ALL
SELECT '102', 4 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '102')
UNION ALL
SELECT '103', 4 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '103')
UNION ALL
SELECT '104', 6 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '104')
UNION ALL
SELECT '201', 2 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '201')
UNION ALL
SELECT '202', 4 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '202')
UNION ALL
SELECT '203', 4 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '203')
UNION ALL
SELECT '301', 8 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '301')
UNION ALL
SELECT '302', 4 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '302')
UNION ALL
SELECT '303', 2 WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_tables WHERE table_number = '303');
