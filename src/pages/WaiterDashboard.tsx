...
const WaiterDashboard = () => {
  ...
  const checkAuth = async () => {
    ...
    if (!profileData || !["waiter", "admin", "owner"].includes(profileData.role)) {
    ...
    const { data: orders, error } = await supabase
      ...
      .select(`
        *,
        ...
        order_items(*)
      `)
      .in("status", ["sent_to_kitchen", "preparing", "ready", "served"])
                    ...
                    {tables.map((table) => (
                      <Card
                        ...
                        className={`cursor-pointer transition-all hover:shadow-lg ${
                          table.is_booked ? "border-muted opacity-60" : "border-border"
                        }`}
                        onClick={() => {
                          if (!table.is_booked) {
